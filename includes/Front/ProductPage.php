...
class ProductPage {
	...
	/**
	 * Sobrescreve o canonical tag para URLs de modelo ativo.
	 *
	 * @return void
	 */
	protected function register_canonical_overrides(): void {
		...
		if ( defined( 'WPSEO_VERSION' ) ) {
			add_filter( 'wpseo_canonical', function( $canonical ) use ( $model ) {
				return esc_url( home_url( add_query_arg( 'spm_model', $model->get_slug() ) ) );
			} );
		}
		...
		if ( ! defined( 'WPSEO_VERSION' ) && ! defined( 'RANK_MATH_VERSION' ) ) {
			add_action( 'wp_head', function() use ( $model ) {
				echo '<link rel="canonical" href="' . esc_url( home_url( add_query_arg( 'spm_model', $model->get_slug() ) ) ) . '" />' . "\n";
			}, 1 );
		}
	}
